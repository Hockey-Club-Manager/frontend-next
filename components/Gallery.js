...
import {token2symbol, getTokenOptions, handleOffer, getObjects, getMarketContract} from '../utils/near';
...
export async const Gallery = async ({app, views, update, loading, dispatch}) => {
    const {tab, sort, filter} = app;
    const {tokens, sales, allTokens, marketStoragePaid} = views;

    let nftContract, wallet;
    await getObjects().then(r => {
        const {wallet: _wallet} = r;
        wallet = _wallet;
        nftContract = getMarketContract(_wallet);
    });
    ...
    if (token) {
        return <Token {...{dispatch, wallet, token}} />;
    }
    ...
    return <>
        {
            ...
            <center>
                {
                    tab !== 2 && <button onClick={() => update('app.filter', filter === 2 ? 1 : 2)}
                                         style={{background: '#fed'}}>{filter === 1 ? 'All' : 'Sales'}</button>
                }
                <button onClick={() => update('app.sort', sort === 2 ? 1 : 2)}
                        style={{background: sort === 1 || sort === 2 ? '#fed' : ''}}>Date {sort === 1 && '⬆️'}{sort === 2 && '⬇️'}</button>
                {
                    tab !== 2 && <button onClick={() => update('app.sort', sort === 4 ? 3 : 4)}
                                         style={{background: sort === 3 || sort === 4 ? '#fed' : ''}}>Price {sort === 3 && '⬆️'}{sort === 4 && '⬇️'}</button>
                }
            </center>
        }
        {
            tab === 1 && market.map(({
                ...
                <div key={token_id} className="item">
                    <img src={media}
                         onClick={() => history.pushState({}, '', window.location.pathname + '?t=' + token_id)}/>
                        ...
                        Object.keys(sale_conditions).length > 0 && <>
                            ...
                            }
                            {
                                accountId.length > 0 && accountId !== owner_id && <>
                                    ...
                                    <button onClick={() => handleOffer(token_id, offerToken, offerPrice)}>Offer</button>
                                </>
                            }
                        </>
                    }
                    {
                        Object.keys(bids).length > 0 && <>
                            <h4>Offers</h4>
                            {
                                Object.entries(bids).map(([ft_token_id, ft_token_bids]) => ft_token_bids.map(({
                                    ...
                                    </div>
                                    {
                                        accountId === owner_id &&
                                        <button
                                            onClick={() => handleAcceptOffer(token_id, ft_token_id)}>Accept</button>
                                    }
                                </div>))
                            }
                        </>
